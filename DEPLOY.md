...
Start       npm start
Tests       npm test   →  8 files, 95 tests
```
...
| `PYTH_MAX_AGE_SECONDS` | Defaults to 600 on devnet, 180 on mainnet and `localnet-fork`. A price older than this is shown as stale, never as a number. |
| `PYTH_RPC_URL` | Where Pyth accounts are read. Default: the app's RPC on devnet/mainnet; **live mainnet on `localnet-fork`**, because a fork's cloned Pyth accounts never update. Set it to the fork's RPC to read the clones instead. |
