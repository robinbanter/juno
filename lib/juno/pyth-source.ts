 ...
 *
 * `localnet-fork` is a local validator seeded from mainnet. Its Pyth accounts
 ...
 *
 * Reads the cluster env value directly rather than through `cluster()`, so
 * `localnet-fork` works here before `Cluster` learns about it.
 */
...
export function pythSource(env: Record<string, string | undefined> = process.env): PythSource {
  const clusterName = env.NEXT_PUBLIC_SOLANA_CLUSTER?.trim();
  const override = env.PYTH_RPC_URL?.trim() || null;
  const network: PythNetwork =
    clusterName === "mainnet-beta" || clusterName === "localnet-fork" ? "mainnet-beta" : "devnet";
  const rpc = override ?? (clusterName === "localnet-fork" ? MAINNET_RPC : null);
  return {
    network,
    rpc,
