...
| **Tests** | 95 unit tests across 8 files, incl. all four presets validated by Meteora's own `validateConfigParameters`. Was 140 before the Norr purge took the tests covering deleted code — see [README.md](./README.md). |
...
| `PYTH_MAX_AGE_SECONDS` | Optional; 600 on devnet, 180 on mainnet/`localnet-fork`. Older prices are shown as stale, never as a number. |
| `PYTH_RPC_URL` | Optional. On `localnet-fork` Pyth is read from live mainnet by default (fork clones freeze at startup); set this to read elsewhere. |
