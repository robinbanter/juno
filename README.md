...
| **Tests** | **127 unit tests across 12 files**, passing — including all four presets asserted against Meteora's own `validateConfigParameters`, and both deprecated paths (DAMM v1, `RateLimiter`) asserted unused |
...
```bash
npm test                 # everything
npm run test:unit        # 127 tests, ~1s — the curve presets live here
...
routes — stopped existing. Nothing that guards live code was removed. Of the 127,
**104 are Juno's own** (curves 13, format 13, indexer 17, Pyth reader + NAV band 29,
issuer tooling 9, mainnet-fork config 10, wallet sessions 9, slippage guard 4) and
23 cover shared infrastructure.
...
# Every swap's minimum-out comes from a real quote at the UI's 100 bps tolerance
# (--slippage-bps to change it; --simulate to dry-run). --no-slippage-protection
# sends a zero minimum, which fills at ANY price: it warns, and is refused on mainnet.
